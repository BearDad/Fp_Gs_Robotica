...
\clearpage
  ...
  \begin{quote}
	  ...
	  \begin{paracol}{2}
		  \incfigpdf[0.9]{SEHN_ejercicio-9}
		  \switchcolumn
		\begin{itemize}
			\vfill
  ...
  \begin{quote}
  % ✍️ Write your response here
	  Consiste en crear un circuito cerrado con todas las masas metálicas de la instalación y en contacto directo del circuito 
	  con la tierra.
  \end{quote}
  ...
  \begin{quote}
  % ✍️ Write your response here
	  Se denomina conducto de protección al conductor de tierra.
  \end{quote}
  ...
  % ✍️ Write your response here
	  \begin{itemize}
		  \item{\boldit{Tipo 1:}} Basto
		  \item{\boldit{Tipo 2:}} Medio
		  \item{\boldit{Tipo 3:}} Fino
	  \end{itemize}
  \end{quote}
  ...
  \begin{quote}
  % ✍️ Write your response here
	  Debemos de tener en cuenta la curva de actuación que necesitamos y el amperaje que puede soportar el magnetotérmico.
  \end{quote}
  ...
  \begin{quote}
	  Fallo eléctrico que provoca un pico de corriente extremo e instantáneo en la línea, ocurre cuando dos conductores de 
	  distinto potencial entran en contacto directo entre si puede provocar incendios y la destrucción de la instalación.
  \end{quote}

  \item \boldit{¿Cómo se comprueba si un fusible está o no fundido? ¿Cómo se interpreta dicha comprobación?}

  \begin{quote}
	  Para comprobar si un fusible esta o no fundido debemos usar la función de continuidad de nuestro multímetro y comprobaremos que el filamento
	  del interior del fusible tenga continuidad lo cual significa que no esta fundido.
  \end{quote}

  \item \boldit{¿Para qué sirve el botón de prueba de un diferencial? ¿En qué condiciones actúa?}

  \begin{quote}
	  El botón de prueba sirve para comprobar el correcto funcionamiento del diferencial, al pulsarlo provocamos una corriente de fuga y esta hace 
	  que salte el diferencial en caso de que funcione correctamente. Actuara únicamente en caso de que el diferencial funcione correctamente y 
	  tenga corriente
  \end{quote}
  ...
  \begin{quote}
	  Tanto los fusibles como los magnetotérmicos actúan como protección contra sobrecargas, sobreintensidades y cortocircuitos
	  la diferencia principal es que los magnetotérmicos son 
	  rearmables mientras que los fusibles tienen que ser sustituidos por otros.
  \end{quote}
\clearpage
  \item \boldit{¿Qué diferencias hay entre una sobrecarga y un cortocircuito?}

  \begin{quote}
	  Una sobrecarga es cuando la corriente de la linea supera el máximo que aguanta la línea. Un cortocircuito en cambio es cuando la fase entra
	  en contacto directo con el neutro y provoca una corriente extremadamente alta e inmediata, el cortocircuito también puede ser provocado por
	  el contacto entre fases en un circuito trifásico o por el contacto de la fase y la tierra.
  \end{quote}
  ...
  \item \boldit{Explica las dos formas de disparo de un interruptor magnetotérmicos.}
